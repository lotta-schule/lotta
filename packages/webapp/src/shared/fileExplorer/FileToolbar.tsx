...
export const FileToolbar = React.memo(() => {
  ...
  const uploadTotalProgress =
    uploads
      ...
      .reduce((prevProgress, currProgress) => prevProgress + currProgress, 0) /
    uploadLength;

  const showFileEditingButtons =
    state.mode === FileExplorerMode.ViewAndEdit && state.markedFiles.length > 0;
      ...
      <Toolbar className={styles.toolbar}>
        ...
        <div className={styles.actions}>
          <Collapse visible={uploadLength > 0}>
            <Tooltip label={`${uploadLength} Dateien werden hochgeladen`}>
              <Button
                aria-label={`${uploadLength} Dateien werden hochgeladen`}
                onClick={() => dispatch({ type: 'showActiveUploads' })}
              ...
              >
                ...
                <Badge
                  value={
                    uploads.filter((u) => u.error).length ? '!' : uploadLength
                  }
                />
              </Button>
            </Tooltip>
              ...
              <Collapse visible={showFileEditingButtons}>
                <Tooltip label="Dateien verschieben">
                  <Button
                    ...
                    onClick={() => dispatch({ type: 'showMoveFiles' })}
          ...
          {File.canCreateDirectory(
            ...
            <Tooltip label="Ordner erstellen">
              <Button
                ...
                onClick={() => dispatch({ type: 'showCreateNewFolder' })}
            ...
            <Collapse visible={state.currentPath.length > 1}>
              <Tooltip
                label={`Info-Leiste für Dateien und Ordner ${
                  state.detailSidebarEnabled ? 'ausblenden' : 'einblenden'
                }`}
              >
                <Button
