          ...
          if (lowestSelectedFileIndex !== null && lowestSelectedFileIndex > 0) {
            ...
            if (e.shiftKey) {
              dispatch({
                ...
                files: [
                  ...state.markedFiles,
                  filteredSortedFiles[lowestSelectedFileIndex - 1],
                ],
              });
            } else {
              dispatch({
                type: 'markSingleFile',
                file: filteredSortedFiles[lowestSelectedFileIndex - 1],
              });
            }
          }
        }
      ...
      if (e.key === 'ArrowDown') {
        if (state.markedFiles.length) {
          if (
            ...
            highestSelectedFileIndex < filteredSortedFiles.length - 1
          ) {
            e.preventDefault();
            if (e.shiftKey) {
              dispatch({
                ...
                files: [
                  ...state.markedFiles,
                  filteredSortedFiles[highestSelectedFileIndex + 1],
                ],
              });
            } else {
              dispatch({
                type: 'markSingleFile',
                file: filteredSortedFiles[highestSelectedFileIndex + 1],
              });
            }
          }
        }
  ...
  return (
    ...
    >
      {!isDragAccept && isDragActive && (
        <div className={styles.dragHelpText}>Dateien hierher ziehen</div>
      )}
      {isDragAccept && (
          ...
          <tr>
                  ...
                  <Checkbox
                    ...
                    isSelected={every(
                      filteredSortedFiles.filter(
                        (f) => f.fileType !== FileModelType.Directory
                      ),
                      (f) => state.selectedFiles.includes(f)
                    )}
                    onChange={(isSelected) => {
                      dispatch({
                        ...
                        files: isSelected
                          ? uniqBy(
                              [...state.selectedFiles, ...filteredSortedFiles],
                              'id'
                            )
                          : state.selectedFiles.filter(
                              (selectedFile) =>
                                !filteredSortedFiles.includes(selectedFile)
                            ),
                      });
                    }}
                  />
            ...
            <td>
              {isLoading && (
                <CircularProgress
                  ...
                  isIndeterminate
                  aria-label={'Dateien für diesen Ordner werden geladen'}
                />
              )}
              {!isLoading && state.currentPath.length > 1 && (
                <Button
                  small
                  icon={<Icon icon={faCircleChevronLeft} size={'lg'} />}
                  onClick={() =>
                    dispatch({
                      type: 'setPath',
            ...
            <td>
              {!isLoading && state.currentPath.length < 2 && (
                <strong>Wähle einen Ordner um Dateien hochzuladen.</strong>
              )}
              {state.currentPath.length > 1 && 'Dateiname'}
            </td>
            {state.mode === FileExplorerMode.ViewAndEdit && <td>&nbsp;</td>}
          </tr>
        </thead>
        <tbody>
            ...
            <FileTableRow
              ...
              onMark={(e) => {
                if (state.mode === FileExplorerMode.Select) {
                  ...
                  if (state.selectedFiles.find((sf) => sf.id === file.id)) {
                    ...
                    dispatch({
                      type: 'setSelectedFiles',
                      files: [...state.selectedFiles, file],
                    });
                  }
                } else {
          ...
          {state.currentPath.length > 1 &&
            data?.files.length === 0 &&
            data?.directories.length === 0 && <EmptyDirectoryTableRow />}
        </tbody>
      </Table>
      <FileTableFooter />
    </div>
