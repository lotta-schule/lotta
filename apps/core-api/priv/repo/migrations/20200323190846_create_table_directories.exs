defmodule Api.Repo.Migrations.CreateTableDirectories do
  ...
  def up do
    ...
    |> Enum.map(fn file ->
      root_directory = if file.is_public do
        Api.Repo.one!(from d in Api.Accounts.Directory, where: d.tenant_id == ^file.tenant_id and is_nil(d.user_id) and d.name == "schulweite Dateien")
      else
        file_type = case file.file_type do
          "image" ->
            "Meine Bilder"
          "video" ->
            "Meine Videos"
        ...
        Api.Repo.one!(from d in Api.Accounts.Directory, where: d.tenant_id == ^file.tenant_id and d.user_id == ^file.user_id and d.name == ^file_type)
      end
      parent_directory = file.path
      |> String.split("/")
      |> Enum.filter(&(String.length(&1) > 0))
      |> Enum.reduce(root_directory, fn dirname, parent_directory ->
        query = from(d in Api.Accounts.Directory)
        |> where([d], d.name == ^dirname and d.tenant_id == ^file.tenant_id)
        |> (fn query ->
          query = case file.is_public do
            true -> where(query, [d], is_nil(d.user_id))
            false -> where(query, [d], not is_nil(d.user_id))
          end
          query = case parent_directory do
            %{id: parent_directory_id} -> where(query, [d], d.parent_directory_id == ^parent_directory_id)
            _ -> where(query, [d], is_nil(d.parent_directory_id))
          end
          query
        end).()
