defmodule Api.Repo.Migrations.ChangeUserEmailsToHaveLowercaseIndex do
  use Ecto.Migration
  import Ecto.Query
  alias Api.Repo
  ...
  def up do
    ...
    {:ok, %{rows: duplicates}} =
      Api.Repo.query("""
        SELECT id, nickname, name, inserted_at, email, last_seen FROM users  WHERE UPPER(email) IN (
          SELECT UPPER(email) FROM users GROUP BY UPPER(email) HAVING COUNT(*) > 1
    ...
    Enum.map(users_grouped, fn users ->
      ...
      {n, _} =
        Repo.update_all(
          from(repo in "article_users",
            where: repo.user_id in ^del_users and not (repo.article_id in ^keepers_article_ids)
          ),
          set: [user_id: keep_user.id]
        )
      ...
      {n, _} =
        Repo.update_all(
          from(repo in "user_user_group",
            where: repo.user_id in ^del_users and not (repo.user_group_id in ^keepers_user_groups)
          ),
          set: [user_id: keep_user.id]
        )
          ...
          from(repo in "users_enrollment_tokens",
            where:
              repo.user_id in ^del_users and
                not (repo.enrollment_token in ^keepers_user_enrollment_tokens)
          ),
          set: [user_id: keep_user.id]
        )
