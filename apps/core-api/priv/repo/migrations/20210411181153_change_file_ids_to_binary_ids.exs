defmodule Api.Repo.Migrations.ChangeFileIdsToBinaryIds do
  use Ecto.Migration
  ...
  def up do
    ...
    Repo.query!(
      "UPDATE file_conversions SET file_uuid=files.uuid FROM files WHERE file_conversions.file_id=files.id;"
    )

    Repo.query!(
      "UPDATE files SET parent_directory_uuid=directories.uuid FROM directories WHERE files.parent_directory_id=directories.id;"
    )

    Repo.query!(
      "UPDATE directories SET parent_directory_uuid=parent_directories.uuid FROM directories as parent_directories WHERE directories.parent_directory_id=parent_directories.id;"
    )

    Repo.query!(
      "UPDATE users SET avatar_image_file_uuid=files.uuid FROM files WHERE users.avatar_image_file_id=files.id;"
    )

    Repo.query!(
      "UPDATE categories SET banner_image_file_uuid=files.uuid FROM files WHERE categories.banner_image_file_id=files.id;"
    )

    Repo.query!(
      "UPDATE widgets SET icon_image_file_uuid=files.uuid FROM files WHERE widgets.icon_image_file_id=files.id;"
    )

    Repo.query!(
      "UPDATE articles SET preview_image_file_uuid=files.uuid FROM files WHERE articles.preview_image_file_id=files.id;"
    )

    Repo.query!(
      "UPDATE content_module_file SET file_uuid=files.uuid FROM files WHERE content_module_file.file_id=files.id;"
    )

    Repo.query!(
      "UPDATE configuration SET file_value_uuid=files.uuid FROM files WHERE configuration.file_value_id=files.id;"
    )
    ...
    Repo.query!("ALTER TABLE files ADD PRIMARY KEY (id)")
    Repo.query!("ALTER TABLE directories ADD PRIMARY KEY (id)")
