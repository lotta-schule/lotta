defmodule Api.Repo.Migrations.AddMetadataToFilesAndFileConversions do
  require Logger
  ...
  def up do
    ...
    Api.Repo.all(
      from(fc in Api.Accounts.FileConversion,
    ...
    |> Enum.each(fn [id, remote_location] ->
      case HTTPoison.head(remote_location) do
        {:ok, %{headers: headers}} ->
          ...
          if filesize do
            from(fc in Api.Accounts.FileConversion, where: fc.id == ^id)
            |> Api.Repo.update_all(set: [filesize: String.to_integer(filesize)])
          else
