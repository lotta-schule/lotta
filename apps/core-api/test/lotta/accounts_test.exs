defmodule Lotta.AccountsTest do
  ...
  describe "users" do
    ...
    test "register_user_by_mail/1 with valid data creates a user with a password", %{tenant: t} do
      assert {:ok, %User{} = user} =
               Accounts.register_user_by_mail(t, %{
                 email: "some@email.de",
                 name: "Alberta Smith",
                 nickname: "TheNick",
                 class: "5",
    ...
    test "register_user_by_mail/1 with invalid data returns error changeset", %{tenant: t} do
      assert {:error, %Ecto.Changeset{}} =
               Accounts.register_user_by_mail(t, %{email: nil, name: nil, nickname: nil})
    end
    ...
    test "get_or_create_eduplaces_user/1 should create as well as find a user by its eduplaces_id",
      ...
      created_user = insert(:user, eduplaces_id: eduplaces_id)
    ...
    test "get_user_by_eduplaces_id/1 returns user with matching eduplaces_id" do
      ...
      user = insert(:user, eduplaces_id: eduplaces_id, email: "eduplace-test@test.com")
    ...
    test "list_users_by_eduplaces_ids/1 returns users with matching eduplaces_ids" do
      user1 = insert(:user, eduplaces_id: "edu-1", email: "edu1@test.com")
      user2 = insert(:user, eduplaces_id: "edu-2", email: "edu2@test.com")
      _user3 = insert(:user, eduplaces_id: "edu-3", email: "edu3@test.com")
    ...
    test "update_profile/2 with invalid data returns error changeset" do
      ...
      assert {:error, %Ecto.Changeset{}} =
               Accounts.update_profile(user, %{email: nil, name: nil, nickname: nil})
  ...
  describe "list_admin_users/1" do
    test "returns users that are members of admin groups", %{tenant: tenant} do
      ...
      regular_group = insert(:group)
      admin_user = insert(:user, email: "admin@test.com", name: "Admin User")
      regular_user = insert(:user, email: "user@test.com", name: "Regular User")

      Accounts.update_user(admin_user, %{groups: [admin_group]})
      Accounts.update_user(regular_user, %{groups: [regular_group]})

      admin_emails = Accounts.list_admin_users(tenant) |> Enum.map(& &1.email)
    ...
    test "returns empty list when no admin groups exist", %{tenant: tenant} do
      ...
      user_emails = Accounts.list_admin_users(tenant) |> Enum.map(& &1.email)
      refute "user@test.com" in user_emails
    end

    test "returns distinct users when user is in multiple admin groups", %{tenant: tenant} do
      admin_group1 = insert(:admin_group)
      admin_group2 = insert(:admin_group)
      admin_user = insert(:user, email: "multi-admin@test.com", name: "Multi Admin")

      Accounts.update_user(admin_user, %{groups: [admin_group1, admin_group2]})

      matching_users =
        Accounts.list_admin_users(tenant)
        |> Enum.filter(&(&1.email == "multi-admin@test.com"))
    ...
    test "works without tenant parameter using default prefix" do
      admin_group = insert(:admin_group)
      admin_user = insert(:user, email: "default-admin@test.com")
      Accounts.update_user(admin_user, %{groups: [admin_group]})

      admin_emails = Accounts.list_admin_users() |> Enum.map(& &1.email)
  ...
  describe "search_user/3" do
    test "finds user by exact email match" do
      _user = insert(:user, email: "exact@test.com", name: "Exact User")
    ...
    test "finds users by partial name match" do
      _user1 = insert(:user, email: "user1@test.com", name: "John Smith")
      _user2 = insert(:user, email: "user2@test.com", name: "Jane Smith")
      _user3 = insert(:user, email: "user3@test.com", name: "Bob Jones")

      result_names = Accounts.search_user("Smith", nil, nil) |> Enum.map(& &1.name)
    ...
    test "finds users by partial nickname match" do
      _user = insert(:user, email: "nick@test.com", name: "User", nickname: "SuperNick")

      assert Enum.any?(Accounts.search_user("Super", nil, nil), &(&1.nickname == "SuperNick"))
    end

    test "filters by group IDs" do
      group1 = insert(:group)
      group2 = insert(:group)
      user1 = insert(:user, email: "group1@test.com", name: "Group1 User")
      user2 = insert(:user, email: "group2@test.com", name: "Group2 User")

      Accounts.update_user(user1, %{groups: [group1]})
      Accounts.update_user(user2, %{groups: [group2]})

      result_emails = Accounts.search_user(nil, [group1.id], nil) |> Enum.map(& &1.email)
    ...
    test "includes users without groups when nil group ID is specified" do
      group = insert(:group)
      user_with_group = insert(:user, email: "with-group@test.com")
      _user_without_group = insert(:user, email: "without-group@test.com")

      Accounts.update_user(user_with_group, %{groups: [group]})

      result_emails = Accounts.search_user(nil, [nil], nil) |> Enum.map(& &1.email)
    ...
    test "filters by last seen after date" do
      ...
      user = insert(:user, email: "recent@test.com")

      user |> Ecto.Changeset.change(%{last_seen: past_date}) |> Repo.update()

      result_emails =
        Accounts.search_user(nil, nil, {:after, DateTime.utc_now()}) |> Enum.map(& &1.email)
    ...
    test "filters by last seen before date" do
      ...
      result_emails = Accounts.search_user(nil, nil, {:before, past_date}) |> Enum.map(& &1.email)
  ...
  describe "list_users_for_group/1" do
    test "returns users assigned to group" do
      group = insert(:group)
      user1 = insert(:user, email: "assigned1@test.com")
      user2 = insert(:user, email: "assigned2@test.com")

      Accounts.update_user(user1, %{groups: [group]})
      Accounts.update_user(user2, %{groups: [group]})

      user_emails = Accounts.list_users_for_group(group) |> Enum.map(& &1.email)
    ...
    test "returns empty list for group with no users" do
      group = insert(:group)
      assert Accounts.list_users_for_group(group) == []
    end
  end
  ...
  describe "list_users_for_groups/1" do
    test "returns users from multiple groups" do
      group1 = insert(:group)
      group2 = insert(:group)
      user1 = insert(:user, email: "multi1@test.com")
      user2 = insert(:user, email: "multi2@test.com")

      Accounts.update_user(user1, %{groups: [group1]})
      Accounts.update_user(user2, %{groups: [group2]})

      user_emails = Accounts.list_users_for_groups([group1, group2]) |> Enum.map(& &1.email)
    ...
    test "returns unique users when user is in multiple queried groups" do
      group1 = insert(:group)
      group2 = insert(:group)
      user = insert(:user, email: "overlap@test.com")
      Accounts.update_user(user, %{groups: [group1, group2]})

      matching_users =
        Accounts.list_users_for_groups([group1, group2])
        |> Enum.filter(&(&1.email == "overlap@test.com"))
  ...
  describe "list_users_without_groups/0" do
    test "returns users not assigned to any group" do
      ...
      user_emails = Accounts.list_users_without_groups() |> Enum.map(& &1.email)
    ...
    test "returns empty list when all users are in groups" do
      ...
      Enum.each(existing_users, fn user -> Accounts.update_user(user, %{groups: [group]}) end)
      assert Accounts.list_users_without_groups() == []
    end
  end

  describe "password reset" do
    test "request_password_reset/1 creates reset token for existing user" do
      user = insert(:user, email: "reset@test.com")
    ...
    test "request_password_reset/1 is case insensitive for email" do
      user = insert(:user, email: "CaseInsensitive@test.com")
    ...
    test "find_user_by_reset_token/2 returns user for valid token" do
      user = insert(:user, email: "token-test@test.com")
    ...
    test "find_user_by_reset_token/2 returns error for invalid token" do
      _user = insert(:user, email: "invalid-token@test.com")
  ...
  describe "user_groups" do
    ...
    test "set_group_members/2 sets the members of a group" do
      group = insert(:group)
      user1 = insert(:user, email: "member1@test.com")
      user2 = insert(:user, email: "member2@test.com")
    ...
    test "set_group_members/2 replaces existing members" do
      group = insert(:group)
      user1 = insert(:user, email: "old1@test.com")
      user2 = insert(:user, email: "old2@test.com")
      user3 = insert(:user, email: "new1@test.com")
    ...
    test "set_group_members/2 can set empty member list" do
      group = insert(:group)
      user1 = insert(:user, email: "remove@test.com")
  ...
  describe "additional user functions" do
    test "update_user/2 updates user groups" do
      user = insert(:user, email: "admin-update@test.com")
      group = insert(:group)

      assert {:ok, updated_user} = Accounts.update_user(user, %{groups: [group]})
    ...
    test "see_user/1 updates last_seen timestamp" do
      user = insert(:user, email: "seen@test.com")

      user |> Ecto.Changeset.change(%{last_seen: nil}) |> Repo.update()
    ...
    test "get_user_by_email/1 finds user by email" do
      user = insert(:user, email: "find-by-email@test.com")
    ...
    test "list_user_groups/0 returns all user groups ordered correctly" do
      _group1 = insert(:admin_group)
      _group2 = insert(:group)
    ...
    test "get_or_create_eduplaces_user/2 finds existing user" do
      existing_user = insert(:user, eduplaces_id: "existing-123")
  ...
  describe "device management" do
    test "list_devices/1 returns all devices for user" do
      user = insert(:user, email: "device-owner@test.com")
    ...
    test "get_device/1 returns specific device" do
      user = insert(:user, email: "device-getter@test.com")
    ...
    test "delete_device/1 removes device" do
      user = insert(:user, email: "device-deleter@test.com")
  ...
  describe "user_tokens" do
    test "it should add a device for a user" do
      ...
      assert {:ok, %UserDevice{platform_id: "ios/123-123-123"}} =
               Accounts.register_device(user, %{
                 custom_name: "Test",
    ...
    test "it should perform an upsert when inserting a device that already exists" do
      ...
      assert {:ok, %UserDevice{id: id, platform_id: "ios/123-456-789", push_token: "apns/dadada"}} =
               Accounts.register_device(user, %{
                 custom_name: "Test",
      ...
      assert {:ok, %UserDevice{id: ^id, platform_id: "ios/123-456-789"}} =
               Accounts.register_device(user, %{
                 platform_id: "ios/123-456-789",
                 device_type: "phone",
                 model_name: "iphone16,1",
