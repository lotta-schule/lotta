...
interface ArticlePreviewProps<A extends ArticlePreviewModel> {
  article: A;
  disableLink?: boolean;
  onUpdateArticle?: (article: A) => void;
  disableEdit?: boolean;
  disablePin?: boolean;
...
// Generic over the article shape so the same component serves both preview lists
// (preview-query shape — light authors, no `onUpdateArticle`) and in-place editing
// (full `ArticleModel`, `onUpdateArticle` receiving the same full shape it spreads).
const ArticlePreviewComponent = <A extends ArticlePreviewModel>({
  article,
  disableLink,
  disableEdit,
  disablePin,
  ...
  loadImageEagerly,
  onUpdateArticle,
}: ArticlePreviewProps<A>) => {
  const currentUser = useCurrentUser();

  const [selectedTag, setSelectedTag] = React.useState<string | null>(null);
  const [selectedUser, setSelectedUser] =
    React.useState<UserPreviewModel | null>(null);
  ...
  const [toggleArticlePin] = useMutation<{ article: ArticleModel }, { id: ID }>(
...
) as typeof ArticlePreviewComponent & { displayName?: string };
