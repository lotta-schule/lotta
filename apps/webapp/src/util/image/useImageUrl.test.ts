  ...
  describe('with cloudimage config', () => {
    it('should output a diverse sitemap with given width and height', () => {
      ...
      expect(screen.result.current.sizeMap).toEqual({
        '100w':
          'https://my.image/on-path?width=100&height=50&fn=cover&format=webp',
        '200w':
          'https://my.image/on-path?width=200&height=100&fn=cover&format=webp',
        '400w':
          'https://my.image/on-path?width=400&height=200&fn=cover&format=webp',
        '600w':
          'https://my.image/on-path?width=600&height=300&fn=cover&format=webp',
      });
    ...
    it('should output a diverse sitemap with width and aspectRatio', () => {
      ...
      expect(screen.result.current.sizeMap).toEqual({
        ...
        '1000w': 'https://my.image/on-path?width=1000&height=750&fn=cover',
        '1500w': 'https://my.image/on-path?width=1500&height=1125&fn=cover',
      });
      expect(screen.result.current.customStyle).toEqual({
