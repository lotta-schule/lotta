...
const createMocks = (props?: Partial<CategoryModel>) => [{
    request: { query: CreateCategoryMutation, variables: { category: {
        ...
        ...(props?.category && { category: { id: props.category.id } })
    } } },
    result: {
        data: {
            category: {
                ...
                category: props?.category ? {
                    ...
                    hideArticlesFromHomepage: props.category.hideArticlesFromHomepage
                } : null,
            }
        }
    }
...
describe('component/layouts/adminLayout/userManagment/CreateCategoryDialog', () => {
    ...
    it('should reset the input field when dialog is closed and then reopened', () => {
        const screen =
            render(<CreateCategoryDialog isOpen onConfirm={() => {}} onAbort={() => {}} />);
        userEvent.type(screen.getByRole('textbox'), 'Test');
        screen.rerender(<CreateCategoryDialog isOpen={false} onConfirm={() => {}} onAbort={() => {}} />);
        screen.rerender(<CreateCategoryDialog isOpen onConfirm={() => {}} onAbort={() => {}} />);
    ...
    describe('send for main category form', () => {
        it('should hide the category selection', async () => {
            ...
            expect(screen.getByTestId('CategorySelect').parentElement).toHaveStyle({ height: 0 });
        });
    ...
    describe('send for subcategory', () => {
        it('should disable the submit button if no parentCategory is selected', () => {
            ...
            userEvent.type(screen.getByRole('textbox'), 'Test');
            userEvent.click(screen.getByRole('radio', { name: /unterkategorie/i }));
            expect(screen.getByRole('button', { name: /erstellen/ })).toBeDisabled();
        });
        ...
        it('should create an article', async () => {
            const onConfirm = jest.fn(createdCategory => {
                expect(createdCategory.title).toEqual('Test');
                expect(createdCategory.isSidenav).toEqual(false);
                expect(createdCategory.category).toHaveProperty('id', FaecherCategory.id);
            });

            const screen =
                render(
                    <CreateCategoryDialog isOpen onConfirm={onConfirm} onAbort={() => {}} />,
                    {}, { currentUser: SomeUser, additionalMocks: createMocks({ isSidenav: false, category: FaecherCategory }) }
                );

            userEvent.type(screen.getByRole('textbox'), 'Test');
            userEvent.click(screen.getByRole('radio', { name: /unterkategorie/i }));
            await new Promise(resolve => setTimeout(resolve, 500));
            userEvent.click(screen.getByRole('button', { name: /kategorie wählen/i }));
            userEvent.click(await screen.findByRole('option', { name: /fächer/i }));
            userEvent.click(screen.getByRole('button', { name: /erstellen/ }));
    ...
    describe('send for sidenav', () => {
        it('should create an article', async () => {
            ...
            userEvent.type(screen.getByRole('textbox'), 'Test');
            userEvent.click(screen.getByRole('radio', { name: /seitenleistenkategorie/i }));
            userEvent.click(screen.getByRole('button', { name: /erstellen/ }));
