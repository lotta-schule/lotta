    ...
    describe('SearchLayout', () => {
        ...
        it('should issue a search request after typing', async () => {
            const screen = render(
                <SearchLayout />,
                {}, { additionalMocks: [{
                    request: { query: SearchQuery, variables: { searchText: 'Test', options: { categoryId: null } } },
        ...
        describe('filtering', () => {
            it('should open and close search filter', async () => {
                ...
                userEvent.click(screen.getByRole('button', { name: /erweitert/i }));
                await new Promise(resolve => setTimeout(resolve, 500));
                await waitFor(() => {
                    expect(screen.getByRole('heading', { name: /erweitert/i }).parentElement).toBeVisible();
                });
                userEvent.click(screen.getByRole('button', { name: /erweitert/i }));
                await new Promise(resolve => setTimeout(resolve, 750));
                expect(await screen.findByRole('heading', { name: /erweitert/i })).not.toBeVisible();
            });

            it('should add category to filteroptions', async () => {
                const resultFn = jest.fn(() => ({ data: { results: [ComputerExperten] } }));
                const screen =
                    render(
                        <SearchLayout />,
                        {}, {
                            additionalMocks: [{
                                request: { query: SearchQuery, variables: { searchText: 'Test', options: { categoryId: FaecherCategory.id } } },
                                result: resultFn
                            }]
                        }
                    );
                userEvent.click(screen.getByRole('button', { name: /erweitert/i }));
                userEvent.type(screen.getByLabelText('Suchbegriff'), 'Test');
                userEvent.click(screen.getByRole('button', { name: /kategorie/i }));
