...
export const CategorySelect = React.memo<CategorySelectProps>((
    ...
    const getMenuItemForCategory = React.useCallback((category: CategoryModel) => (
        <MenuItem key={category.id} value={category.id}>
            ...
            {(category.category || hideSubCategories) && (
                <span style={category.category ? { paddingLeft: '1.5em' } : undefined}>{category.title}</span>
            )}
        </MenuItem>
    ), [hideSubCategories]);
    ...
    const menuItems = mainCategories
        .map(category =>
            hideSubCategories ? [category] : [category].concat(categories.filter(c => c.category?.id === category.id))
        )
        .flat()
        .map(getMenuItemForCategory);

    return (
        <FormControl disabled={disabled} variant={'outlined'} fullWidth data-testid="CategorySelect" className={className}>
