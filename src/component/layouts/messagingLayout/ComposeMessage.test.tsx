...
describe('component/layouts/messagingLayout/ComposeMessage', () => {
    ...
    it('should render the component', () => {
        render(
            <ComposeMessage
                threadRepresentation={{ messageType: ChatType.DirectMessage, counterpart: SomeUserin }}
            />
        );
    });
    ...
    describe('send form', () => {
        ...
        it('should send a user a comment', async () => {
            ...
            const additionalMocks = [{
                request: {
                    ...
                    variables: { message: { content: 'Hallo!', recipientUser: { id: SomeUserin.id }, recipientGroup: undefined } }
                },
                result: () => {
                    didCallMutation = true;
                    return {
                        data: {
                            id: 1, senderUser: SomeUser, recipientUser: SomeUserin, recipientGroup: null,
                            insertedAt: new Date().toString(), updatedAt: new Date().toString(),
                        }
                    };
                }
            }];
        ...
        it('should not send form on ENTER when SHIFT modifier is pressed', async () => {
            ...
            userEvent.type(screen.getByRole('textbox'), 'Hallo!{shift}{enter}Zweite Zeile');
