...
import { Dialog, DialogTitle, IconButton, List, ListItem, ListItemAvatar, ListItemText, ListItemSecondaryAction, makeStyles } from '@material-ui/core';
...
export const FileUsageModal = memo(() => {
    ...
    const [{ showFileUsage, markedFiles }, dispatch] = useContext(fileExplorerContext);

    const { data } = useQuery<{ file: FileModel }, { id: ID }>(GetFileDetailsQuery, {
        variables: { id: markedFiles[0]?.id },
        skip: !showFileUsage || markedFiles.length !== 1 || !markedFiles[0]
    });

    const hasSecondaryAction = (usage: FileModelUsageLocation) => usage.article || usage.category || usage.user?.id === current_user?.id;

    const getSecondaryActionCallback = (usage: FileModelUsageLocation) => (e: React.MouseEvent<any>) => {
        if (usage.user) {
            window.open('/profile');
        }
        if (usage.category) {
    ...
    return (
        <Dialog open={showFileUsage && markedFiles.length === 1} onClose={() => dispatch({ type: 'hideFileUsage' })}>
            <DialogTitle>Nutzung der Datei {markedFiles[0]?.filename}</DialogTitle>
            <List>
                    ...
                    <ListItem key={i}>
                        {usage.article?.previewImageFile?.remoteLocation && (
                            <ListItemAvatar>
                                <Img
                                    operation={'cover'}
                                    size={'150x100'}
                                    src={usage.article.previewImageFile.remoteLocation}
                                    alt={`Vorschaubild zu ${usage.article.title}`}
                                />
                            </ListItemAvatar>
                        )}
                        {usage.tenant?.logoImageFile?.remoteLocation && (
                            <ListItemAvatar>
                                <Img
                                    operation={'cover'}
                                    size={'150x100'}
                                    src={usage.tenant.logoImageFile.remoteLocation}
                        ...
                        {hasSecondaryAction(usage) && (
                            <ListItemSecondaryAction>
                                <IconButton onClick={getSecondaryActionCallback(usage)}>
                                    <OpenInNew />
                                </IconButton>
                            </ListItemSecondaryAction>
                        )}
