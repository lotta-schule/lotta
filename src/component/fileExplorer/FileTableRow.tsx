...
export const FileTableRow = memo<FileTableRowProps>(({ file, onMark }) => {
    ...
    const handleEditMenuClickDelete = useCallback((event: MouseEvent<HTMLElement>) => {
        event.stopPropagation();
        setEditMenuAnchorEl(null);
        ...
        <TableRow hover className={clsx({ selected: isMarked })} onClick={onMark}>
            <TableCell>
                {(state.mode === FileExplorerMode.SelectMultiple) && (
                    <Checkbox
                        style={{ padding: 0 }}
                        checked={state.selectedFiles.find(f => f.id === file.id) !== undefined}
                        onChange={(e, checked) => {
                            dispatch({
                ...
                <TableCell>
                    ...
                    >
                        {[
                            ...
                            <MenuItem key={'delete'} onClick={(e) => handleEditMenuClickDelete(e)}><DeleteOutlineOutlined color={'secondary'} />&nbsp;Löschen</MenuItem>
                        ]}
                    </Menu>
                </TableCell>
            )}
