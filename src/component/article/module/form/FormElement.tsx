...
import {
    ButtonGroup, Button, Checkbox, FormControlLabel, TextField, Typography, FormGroup,
    FormControl, RadioGroup, Radio, InputLabel, Select, MenuItem, FormLabel, IconButton
            ...
            if (element.type === 'checkbox') {
                ...
                return (
                    <FormControl required={element.required} component={'fieldset'} fullWidth>
                        <InputLabel htmlFor={`form-select-${element.name!}`}>{element.label}</InputLabel>
                        <Select value={value ?? element.options?.find(o => o.selected)?.value ?? ''} onChange={({ target: { value } }) => onSetValue(value as string)} required={element.required} id={`form-select-${element.name!}`}>
                            {element.options?.map((option, i) => {
                                return (
        ...
        if (element.element === 'input') {
            return (
                <TextField
                    ...
                    inputProps={{ 'aria-label': element.label ?? element.descriptionText ?? element.name }}
                />
            )
        }
                ...
                <>
                    <ButtonGroup disableElevation style={{ width: '100%' }} variant={'contained'} color={'primary'}>
                        ...
                        {currentUser && (
                            <SelectFileButton
                                ...
                                buttonComponentProps={{ style: { flex: '0 0 50%' }, variant: 'contained', color: 'primary' }}
                    ...
                    {value && /^lotta-file-id:\/\/.+/.test(value as string) && (
                        <p style={{ paddingLeft: '1.5em' }}>
                            &nbsp;
                            {JSON.parse((value as string).replace(/^lotta-file-id:\/\//, '')).filename}
                            <IconButton title={'Auswahl entfernen'} onClick={() => onSetValue('')}><Close /></IconButton>
                        </p>
                    )}
                    {value && /^file-upload:\/\/.+/.test(value as string) && (
                        <p style={{ paddingLeft: '1.5em' }}>
                            &nbsp;
                            {JSON.parse((value as string).replace(/^file-upload:\/\//, '')).filename}
                            <IconButton title={'Auswahl entfernen'} onClick={() => onSetValue('')}><Close /></IconButton>
                        </p>
                    )}
                </>
