...
export const CategoryRoute = memo<RouteComponentProps<{ id: string }>>(({ match }) => {
    ...
    const nextFetchCount = useMemo(() => {
        ...
        return Math.round(1.5 * window.innerHeight / defaultElmHeight) + (category?.layoutName === '2-columns' && defaultElmHeight % 2 !== 0 ? 1 : 0);
    }, [category]);
    ...
    const maybeFetchMoreArticles = useCallback(() => {
        if (window.innerHeight + Math.max(window.pageYOffset, document.documentElement.scrollTop) > document.documentElement.offsetHeight - FETCH_MORE_OFFSET) {
            if (data && data.articles && data.articles.length > nextFetchCount - 1 && !isLoading) {
                const lastDate = [...(data?.articles ?? [])]
                    .sort((a1, a2) => new Date(a1.updatedAt).getTime() - new Date(a2.updatedAt).getTime())[0].updatedAt;
                if (lastFetchedElementDate !== lastDate) {
                    try {
                        fetchMore({
                            variables: { filter: { first: nextFetchCount, updated_before: lastDate } },
