...
describe('component/widgets/Schedule', () => {
    ...
    const mocks = [
        {
            request: { query: GetScheduleQuery, variables: { widgetId: VPSchuelerWidget.id, date: undefined } },
            result: () => {
        ...
        {
            request: { query: GetScheduleQuery, variables: { widgetId: VPSchuelerWidget.id, date: '2020-11-13' } },
            result: () => {
        ...
        {
            request: { query: GetScheduleQuery, variables: { widgetId: VPSchuelerWidget.id, date: '2020-11-16' } },
            result: () => {
                didLoadCurrentSchedule = true;
                return { data: getScheduleResponse() };
            }
        ...
        {
            request: { query: GetScheduleQuery, variables: { widgetId: VPSchuelerWidget.id, date: '2020-11-18' } },
            result: () => {
    ...
    describe('Pupil\'s Schedule', () => {
        it('should show an information and a link to profile if user has no class', () => {
            const screen = render(
                <Schedule widget={VPSchuelerWidget} />,
                {},
                { currentUser: SomeUserin, additionalMocks: mocks, useCache: true }
            );
            expect(screen.getByRole('alert', { name: /hast keine klasse/i })).toBeVisible();
        ...
        it('should show a current schedule', async () => {
            ...
            expect(screen.getByRole('row', { name: /8 sm15 XMei HDS2/i })).toBeVisible();
            expect(screen.getByRole('row', { name: /alles anders/i })).toBeVisible();
            expect(screen.getByRole('row', { name: /8 sw1 Wal BSZ2/i })).toBeVisible();
        });
